...
exports.listCourses = async (req, res, next) => {
  try {
    const {
      school, level, type, search, sort = 'popular',
      page = 1, limit = 12, free
    } = req.query;
    ...
    const [[countResult], courses] = await Promise.all([
      query(`SELECT COUNT(*) AS total FROM courses c JOIN schools s ON s.id = c.school_id WHERE ${where.join(' AND ')}`, params.slice(0, -2)),
      query(sql, params),
    ]);
